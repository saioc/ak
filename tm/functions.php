...
if (function_exists('register_sidebar')) {
	register_sidebar(array('name' => '全站侧栏', 'id' => 'widget_right', 'before_widget' => '<div class="wow fadeIn widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3><span>', 'after_title' => '</span></h3>'));
	register_sidebar(array('name' => '首页侧栏', 'id' => 'widget_sidebar', 'before_widget' => '<div class="wow fadeIn widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3><span>', 'after_title' => '</span></h3>'));
	register_sidebar(array('name' => '文章页侧栏', 'id' => 'widget_post', 'before_widget' => '<div class="wow fadeIn widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3><span>', 'after_title' => '</span></h3>'));
	register_sidebar(array('name' => '页面侧栏', 'id' => 'widget_page', 'before_widget' => '<div class="wow fadeIn widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3><span>', 'after_title' => '</span></h3>'));
	register_sidebar(array('name' => '分类/标签/搜索页侧栏', 'id' => 'widget_other', 'before_widget' => '<div class="wow fadeIn widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3><span>', 'after_title' => '</span></h3>'));
}
...
//mp4媒体
function flv_video($atts, $content=null){ 
  return '<video poster="" controls="controls" src="'.$content.'" type="video/mp213" style=" width: 100%; ">浏览器不支持此视频！</video>';
} 
add_shortcode('video','flv_video'); 
//视频封面
function url_image($atts, $content=null){ 
  return '<div class="yincang"> '.$content.'</div>';
} 
add_shortcode('spfm','url_image'); 
//视频网址
function url_video($atts, $content=null){ 
  return '<iframe src="https://mayiwo.xyz/?url='.$content.'" name="iframe"  onload="this.height=iframe.document.body.scrollHeight"  width="100%" height="280px" allowtransparency="true" frameborder="0" scrolling="no" border="0" marginwidth="0" marginheight="0" allowfullscreen="true" style="overflow: hidden;"></iframe> 

  '; 
} 
add_shortcode('sp','url_video'); 
// 月费会员可见
function tt_sc_vip1v( $atts, $content ){
    if( !is_null( $content ) ) :
        if(!is_user_logged_in()){
            $content = '<div class="bg-lr2v contextual-bg bg-info"><i class="tico tico-group"></i>' . __('此处内容需要 <span class="user-login">登录</span> 才可见', 'tt') . '</div>';
        }else{
            global $post;
...
add_shortcode( 'vip', 'tt_sc_vip1v' );
 //视频封面
function embed_swf($atts, $content=null){ 
  return '<p><embed src=" '.$content.'" type="application/x-shockwave-flash" width="100%" height="100%" alt="瞄人缝" style="width: 480px; height: 280px;"></embed></p>';
} 
add_shortcode('embed_swf','embed_swf');

function appthemes_add_qgg_liangdian() {
?>
    <script type="text/javascript">
        if ( typeof QTags != 'undefined' ) {
            QTags.addButton( 'flv_video', 'video', '[video]', '[/video]' );
            QTags.addButton( 'url_video', '视频网址', '[sp]', '[/sp]' );
            QTags.addButton( 'url_image', '视频内容', '[spfm]', '[/spfm]' );
            QTags.addButton( 'tt_sc_vip1v', 'vip', '[vip]', '[/vip]' );
            QTags.addButton( 'embed_swf', 'embed', '[embed_swf]', '[/embed_swf]' );
        } 
    </script>
<?php 
} 
...
function links_nofollow($url) {
    if(strpos($url,'://')!==false && strpos($url,'fuli.hk')===false && !preg_match('/(ed2k|thunder|Flashget|flashget|qqdl):\/\//i',$url)) {
