...
function get_curl($url, $post=0, $referer=0, $cookie=0, $header=0, $ua=0, $nobaody=0)
{
	...
	if($referer){
		if($referer==1){
			curl_setopt($ch, CURLOPT_REFERER, '//m.qzone.com/infocenter?g_f=');
		}else{
...
{
    $url = '//int.dpool.sina.com.cn/iplookup/iplookup.php?format=json&ip=';
...
function send_mail($to, $sub, $msg) {
	...
	if($conf['mail_cloud']==1){
		$url='//api.sendcloud.net/apiv2/mail/send';
		$data=array(
...
function send_sms($phone, $code, $moban='1'){
	global $conf;
	$app=$conf['web_name'];
	$url = '//api.978w.cn/yzmsms/index/appkey/'.$conf['sms_appkey'].'/phone/'.$phone.'/moban/'.$moban.'/app/'.$app.'/code/'.$code;
	$data=get_curl($url);
