...
function tt_download_metabox_callback( $post ) {
    ...
    <p><?php _e( '(Tint2.4后废弃,请按格式迁移至下面的资源区)积分下载资源，格式为 资源1名称|资源1url|资源1价格|下载密码, 资源名称与url以及价格、下载密码用|隔开，一行一个资源记录', 'tt' );?></p>
...
new AsyncEmail();
/* Tint-K */
$server_tint_k = strtolower($_SERVER['HTTP_HOST']);
$server_tint_k_arr = explode('.', $server_tint_k);
    if($server_tint_k_arr[0]=='www'){
        $server_tint_k = implode('.', array_slice($server_tint_k_arr, 1));
    }elseif(count($server_tint_k_arr)==3){
        $server_tint_k = $server_tint_k_arr[1] . '.' . $server_tint_k_arr[2];
    }
global $authorization;
if($_COOKIE[$authorization.'_wordpress'] != tint_ok){
	if($json_get = file_get_contents('http://auth.kuacg.com/product/tint.php?api=API_tint&url='.$server_tint_k.'&value=tint&authorization='.$authorization)){
		$row_json=json_decode($json_get,true);
		if($row_json['code'] != yes){
			exit($row_json['msg']);
		}else{
			setcookie($authorization.'_wordpress',tint_ok,time()+3600*24*7,'/');
		}
	}else{
		exit('<!DOCTYPE html><html lang="zh"><head><meta http-equiv="Content-Type"content="text/html; charset=UTF-8"/><title>连接授权服务器失败</title><link rel="stylesheet" type="text/css" href="https://assets.kuacg.com/wp-content/themes/Tint/assets/css/error-page.css"  /></head><body class="error error-page wp-die"><header class="header special-header"></header><div class="wrapper container no-aside"><div class="row"><div class="main inner-wrap"><h1>连接授权服务器失败</h1><p class="die-msg">请检查主题functions.php文件中授权服务器地址是否填写正确，如果没有购买，请访问<a href="https://item.taobao.com/item.htm?id=558410042432"target="_blank"title="购买Tint主题">淘宝</a>购买。</p><p><a class="btn btn-lg btn-success link-home"id="linkBackHome"href="/"title="Go Back Home"role="button">返回首页</a></p></div></div></div><footer class="footer special-footer"><p><span class="copy">&copy;</span>&nbsp;连接授权服务器失败</p></footer></body></html>'.$json_get);
	}
}